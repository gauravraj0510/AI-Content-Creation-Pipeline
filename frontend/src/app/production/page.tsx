...
export default function ProductionPage() {
  ...
  // Group filtered reels by raw idea
  const groupedReels = filteredReels.reduce((groups, reel) => {
    const rawIdeaId = reel.raw_idea_doc_id;
    if (!groups[rawIdeaId]) {
      groups[rawIdeaId] = {
        rawIdea: reel.rawIdea,
        reels: []
      };
    }
    groups[rawIdeaId].reels.push(reel);
    return groups;
  }, {} as Record<string, { rawIdea: RawIdea; reels: ProductionReel[] }>);

  const groupedReelsArray = Object.values(groupedReels);
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center mb-2">
                        ...
                        <h3 className="text-lg sm:text-xl font-semibold text-white truncate">
                          {group.reels[0]?.reel_title || 'Untitled Reel'}
                        </h3>
                        <span className={`ml-2 px-2 py-1 text-xs rounded-full border ${getScoreColor(group.rawIdea.relevance_score)}`}>
                          {group.rawIdea.relevance_score}
                        </span>
                      </div>
